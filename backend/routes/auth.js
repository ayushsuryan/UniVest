    ...
    try {
      ...
      if (referralCode) {
        try {
          ...
          console.error(
            `❌ Error applying referral code ${referralCode}:`,
            error,
          );
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    sendTokenResponse(
      ...
      200,
      res,
      'Email verified successfully. Welcome to Finance App!',
    );
  }),
);
...
router.get(
  ...
  protect,
  asyncHandler(async (req, res) => {
    // Include referral information in profile
    ...
    profile.referralBalance = user.referralBalance;
    profile.referralTier = user.referralTier;
    profile.referralCount = user.referralCount;
