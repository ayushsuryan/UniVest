...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const maturityPeriodDays =
      parseInt(asset.maturityPeriod.replace(/\D/g, '')) || 30;
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const investments = await Investment.getUserInvestments(
      req.user._id,
      status,
    );
    ...
    const investmentsWithCalcs = investments.map(investment => {
      ...
      investmentObj.returnPercentage =
        investment.investedAmount > 0
          ? (
              (investment.totalReturns / investment.investedAmount) *
              100
            ).toFixed(2)
          : '0.00';
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    investment.cashOutDetails = {
      ...
      finalAmount: investment.currentValue, // Full amount
    };
    ...
    res.status(200).json({
      ...
      data: {
        ...
        isMatured: investment.getDaysLeft() <= 0,
      },
    });
  }),
);
  ...
  asyncHandler(async (req, res) => {
    ...
    if (daysLeft > 0) {
      return res.status(400).json({
        success: false,
        message:
          'Investment has not matured yet. Use early cashout with penalty.',
      });
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    investmentObj.returnPercentage =
      investment.investedAmount > 0
        ? ((investment.totalReturns / investment.investedAmount) * 100).toFixed(
            2,
          )
        : '0.00';
